...
function mostrarHistorial(equipos) {
    let contenido = "";
    if (equipos.length >= 1) {
        $.each(equipos, function (index, equipos) {
            ...
            contenido += '<tr><th scope="row">' + equipos.id + '</th>' +
                    ...
                    '<td>' + equipos.tipo + '</td>'  + equipos.id 
                    + '" disabled ';
            if (equipos.novedad) {
                contenido += 'checked'
            }
            contenido += '></td><td>' + equipos.fechaAlquiler + '</td>' +
                    '<td><button id="devolver-btn" onclick= "devolverequipo(' + equipos.id 
                    + ');" class="btn btn-danger">Devolver Equipo</button></td></tr>';
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result != false) {
                ...
                console.log("Error devolviendo el Equipo");
            }
        }
    });
